				...
				<div v-for='item in list'>
					...
					<div class="text-list one-img-text" v-if="item.article_img!==''">
						<div class="ms-img">
							...
							<div class="mv-icon" v-if="item.a_type===3"><img src="../assets/mv.png" /></div>
							<div class="mv-icon" v-if="item.a_type===4"><img src="../assets/au.png" /></div>
						</div>
						<div class="ms-text">
							<span><router-link tag="a" :to="{path:'/newsDescribe',query:{id:item.article_id}}">{{item.title}}</router-link></span>
							<div class="item-text">
...
<script>
	...
	import mfads from '../components/mfads.vue'
	export default{
		name:'jjyw',
		components:{
			HeaderNav,
		...
		data(){
			return{
				url:"http://192.168.18.82:19501/zxz/listMQFC",
				list:[],
				mfads:{
					src:'../../static/img/qy-img.png',
				},
